...
if [ $? -eq 0 ]; then
  ...
  sudo chown -R ${NB_USER}:users $ROOT_PROJECT_DIRECTORY
else
  echo "no_sudo"
  SUDO=1
fi
...
if [  "`which kubectl`" != "" ]; then
    ...
    chown -R ${NB_USER}:users ${HOME}/.kube
fi
...
if [  "`which git`" != "" ]; then
    ...
    # Fix permissions
    [ -d ~/.cache/git ] && chown -R ${NB_USER}:users ~/.cache/git
    [ -f ~/.gitconfig ] && chown ${NB_USER}:users ~/.gitconfig
