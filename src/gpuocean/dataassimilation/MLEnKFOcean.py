...
class MLEnKFOcean:
    ...
    def assimilate(self, MLOceanEnsemble, obs, Hx, Hy, R, r = 2.5*1e7, relax_factor = 1.0):
        """
        ...
        Y0 = ML_state[0][:,obs_idxs[0][0],obs_idxs[0][1]] + ML_perts[0].T
        Y0mean = np.average(Y0, axis=-1)

        lvl_weight = relax_factor * np.ones(np.prod(X0mean.shape))

        ML_XY += (lvl_weight[:,np.newaxis]* 1/Nes[0]*((X0-X0mean[:,:,:,np.newaxis]).reshape(-1,X0.shape[-1]) @ (Y0 - Y0mean[:,np.newaxis]).T)).reshape(X0mean.shape + (3,)).repeat(2**(numLevels-1),1).repeat(2**(numLevels-1),2).reshape(-1,ML_XY.shape[-1])
        ...
        for l_idx in range(1,numLevels):
            ...
            Y0 = ML_state[l_idx][0][:,obs_idxs[l_idx][0][0],obs_idxs[l_idx][0][1]] + ML_perts[l_idx].T
            Y0mean = np.average(Y0, axis=-1)
            Y1 = ML_state[l_idx][1][:,obs_idxs[l_idx][1][0],obs_idxs[l_idx][1][1]] + ML_perts[l_idx].T
            ...
            ML_XY += (lvl_weight[:,np.newaxis] * ( 1/Nes[l_idx]*((X0-X0mean[:,:,:,np.newaxis]).reshape(-1,X0.shape[-1]) @ (Y0 - Y0mean[:,np.newaxis]).T) - 1/Nes[l_idx]*((X1-X1mean[:,:,:,np.newaxis]).reshape(-1,X1.shape[-1]) @ (Y1 - Y1mean[:,np.newaxis]).T) )).reshape(X0mean.shape + (3,)).repeat(2**(numLevels-l_idx-1),1).repeat(2**(numLevels-l_idx-1),2).reshape(-1,ML_XY.shape[-1])

        ML_HXY = ML_XY.reshape(ML_state[-1][0].shape[:-1] + (3,))[:,obs_idxs[-1][0][0],obs_idxs[-1][0][1],:]
        ...
        ## Update
        ML_state[0] = ML_state[0] + (block_reduce( ML_K.reshape(ML_state[-1][0].shape[:-1]+ (3,)),  block_size=(1,2**(numLevels-1), 2**(numLevels-1), 1), func=np.mean).reshape((np.prod(ML_state[0].shape[:-1]),3)) @ (obs[:,np.newaxis] - ML_state[0][:,obs_idxs[0][0],obs_idxs[0][1]] - ML_perts[0].T)).reshape(ML_state[0].shape)

        for l_idx in range(1,numLevels):
            ML_state[l_idx][0] = ML_state[l_idx][0] + (block_reduce(ML_K.reshape(ML_state[-1][0].shape[:-1]+ (3,)), block_size=(1,2**(numLevels-l_idx-1), 2**(numLevels-l_idx-1), 1), func=np.mean).reshape((np.prod(ML_state[l_idx][0].shape[:-1]),3)) @ (obs[:,np.newaxis] - ML_state[l_idx][0][:,obs_idxs[l_idx][0][0],obs_idxs[l_idx][0][1]] - ML_perts[l_idx].T)).reshape(ML_state[l_idx][0].shape)
            ML_state[l_idx][1] = ML_state[l_idx][1] + (block_reduce(ML_K.reshape(ML_state[-1][0].shape[:-1]+ (3,)), block_size=(1,2**(numLevels-l_idx  ), 2**(numLevels-l_idx  ), 1), func=np.mean).reshape((np.prod(ML_state[l_idx][1].shape[:-1]),3)) @ (obs[:,np.newaxis] - ML_state[l_idx][1][:,obs_idxs[l_idx][1][0],obs_idxs[l_idx][1][1]] - ML_perts[l_idx].T)).reshape(ML_state[l_idx][1].shape)
