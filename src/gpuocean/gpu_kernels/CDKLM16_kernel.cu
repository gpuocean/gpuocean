    ...
    if (ti > 1 && ti < NX+2 && tj > 1 && tj < NY+2) {
        ...
        if (h >= KPSIMULATOR_DEPTH_CUTOFF) {
            ...
            if (R[0][j][i] != CDKLM_DRY_FLAG) {
                ...
                const float2 atm_p_central_diff = atmospheric_pressure_central_diff(atmospheric_pressure_current_arr, atmospheric_pressure_next_arr, atmospheric_pressure_t_,  ti+0.5, tj+0.5, NX+4, NY+4);
